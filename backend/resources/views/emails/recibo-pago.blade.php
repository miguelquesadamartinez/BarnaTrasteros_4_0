...
<body style="font-family: Arial, sans-serif; background: #f8f8f8; color: #222; margin:0; padding:0;">
    <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px #0001;">
        <div style="background:#fcc105;padding:18px 24px 10px 24px;">
            <h2 style="margin:0;color:#111827;font-size:1.5rem;">Recibo de pago</h2>
        ...
        <div style="padding:24px;">
            ...
            <p>Le enviamos adjunto el recibo correspondiente al pago realizado el <strong>{{ \Carbon\Carbon::parse($detalle['fecha_pago'])->format('d/m/Y') }}</strong> por importe de <strong>{{ number_format($detalle['importe'], 2, ',', '.') }} €</strong>.
            ...
            <ul style="padding-left:1.2em;">
                ...
                <li><strong>Importe pagado:</strong> {{ number_format($detalle['importe'], 2, ',', '.') }} €</li>
                @if(!empty($detalle['notas']))
                <li><strong>Notas:</strong> {{ $detalle['notas'] }}</li>
                @endif
            </ul>
